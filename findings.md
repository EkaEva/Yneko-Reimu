...
- Extending TOTP to `wp-login.php` can stay account-scoped by using the existing `_yneko_reimu_totp_enabled` and `_yneko_reimu_totp_secret` user meta after WordPress password authentication succeeds. A permanently visible login form field avoids account-state probing from the form display, while the authenticate filter returns a generic combined password/2FA error for missing or wrong codes.
- Recovery codes fit the same account-scoped model when stored as password hashes under `_yneko_reimu_totp_recovery_codes`, returned in plain text only at generation time, and consumed by deleting the matched hash after successful `wp-login.php` authentication.
- Registration, forgot-password, and profile email verification previously had only basic short cooldowns; v0.2.5 now centralizes authentication email sending behind configurable email/IP/device/global budgets.
- Device-level limits intentionally use a random cookie rather than browser fingerprinting. Only hashed email, IP, and device values are stored in transients and alert logs.
- Security alerts are bounded to the latest 100 events and default to backend badges plus `error_log`; email alerts are optional and throttled because default-on alert mail can amplify an attack.
- Native `wp-login.php` coverage is limited to registration and lost-password hooks in this round. Ordinary password-login brute-force expansion remains out of scope because front-end login already has a separate failure limiter and backend login now has TOTP for enabled accounts.
- A final configurable-surface audit found two admin-friendly gaps worth exposing: administrator SVG upload allowance and comment IP region lookup. Both have security/privacy meaning and can be explained to site owners.
- The SVG upload setting should preserve existing behavior by default, remain administrator-only, and keep `yneko_reimu_allow_svg_uploads` as the developer final override.
- The comment IP region lookup setting should preserve existing behavior by default, but when disabled it must skip the `ipwho.is` request before IP validation, cache lookup, or remote calls; browser/system badges can remain local.
- Developer filters such as feature defaults, asset strategy, security headers, schema graph, content width, and virtual pages are better documented as hooks than moved into the admin UI. Internal compatibility values such as legacy GitHub options, old friend/music data, and virtual comment carrier IDs should stay hidden.
