...
- 评论用户体系：支持登录用户头像、个人资料、个人主页、两步验证、评论编辑/删除、角色特殊标签、自定义标签审核和角色头像框。
...
- 用户设置：用户标签、自定义标签审核、屏蔽词、七种基础特殊标签、角色头像框、用户头像上传、头像审核、头像大小上限、用户标签和头像审核管理。
...
```bash
pwsh tools/package-theme.ps1 -Version v0.1.13
```
...
pwsh tools/package-theme.ps1 -OutputName Yneko-Reimu-v0.1.13.zip
```

随后生成并上传：

```text
Yneko-Reimu-v0.1.13.zip
```
...
推荐 tag 命名使用 `vX.Y.Z`，例如 `v0.1.13`。如果手动输入 `0.1.13`，打包脚本会自动补成 `v0.1.13`。
...
- Comment user system with profile avatars, profile URLs, 2FA, own-comment editing/deletion, role badges, custom-badge review, and role-based avatar frames.
...
- Users: user badges, custom-badge review, blocked labels, seven base special badges, role avatar frames, profile avatar uploads, avatar review, avatar size limit, badge management, and avatar manager.
