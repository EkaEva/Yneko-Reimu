...
贡献代码前请先阅读 [CONTRIBUTING.md](CONTRIBUTING.md)。PR 需要说明是否触碰公开接口、运行了哪些检查，以及 UI、评论、上传、登录、OAuth、设置、PJAX 或发布变更的手动 QA。安全问题请按 [SECURITY.md](SECURITY.md) 私下报告，不要先公开漏洞细节。
...
Read [CONTRIBUTING.md](CONTRIBUTING.md) before opening a pull request. PRs should explain public interface impact, checks run, and manual QA for UI, comments, uploads, auth, OAuth, settings, PJAX, or release changes. Report security issues privately through [SECURITY.md](SECURITY.md) instead of opening a public issue first.
