...
站点 Logo 和站点图标建议继续在 `外观 -> 自定义 -> 站点身份` 中设置，因为那里有 WordPress 原生预览。主题允许管理员上传 SVG 作为 Logo 或站点图标，但 SVG 可能携带脚本或外链，请只上传可信来源文件；如需关闭主题的 SVG 媒体库放行，可在 `外观 -> Yneko-Reimu 设置 -> 安全设置 -> 媒体与隐私` 关闭“允许管理员上传 SVG”。如果想保留 SVG 站点图标，可在 `外观 -> Yneko-Reimu 设置 -> 常规设置` 额外配置 `Favicon / Apple Touch 兜底图`，推荐使用 `512x512` PNG/JPG；主题会保留 SVG favicon，并为不稳定支持 SVG 的浏览器、移动端和聊天软件输出 PNG/JPG fallback。`og:image` 仍建议使用 JPG 或 PNG，不建议使用 WebP，并继续由 Rank Math 等 SEO 插件管理。
...
启用 TOTP 时会同时生成一次性恢复码，后台只显示明文一次，数据库只保存哈希。每个恢复码只能使用一次；重新生成恢复码会让旧码全部失效，关闭 TOTP 会清空恢复码。
...
`安全设置` 默认开启认证邮件风控，覆盖主题前台注册验证码、忘记密码验证码、资料邮箱验证码，以及 `wp-login.php` 原生注册/忘记密码入口。默认限额为同一邮箱 `3/小时、8/天`，同一 IP `10/小时、30/天`，同一设备 `5/小时、15/天`，全站每日 `100` 封；设备限制使用随机 Cookie，不做浏览器指纹。触发限额、全站预算预警或邮件发送失败时，会在安全页和后台菜单角标中显示报警，默认只写入后台角标和 `error_log`，不默认发送管理员邮件。

`安全设置 -> 媒体与隐私` 还提供两个默认保持现有行为的开关：管理员 SVG 上传、评论 IP 地区信息。关闭评论 IP 地区信息后，主题不会请求 `ipwho.is`，评论环境标签只保留浏览器和系统信息。
...
- 视觉资产：鼠标指针四个核心槽位、加载动画图片/文案/尺寸/旋转、回到顶部图标、赞助按钮图标。
- 排版与布局密度：正文/标题/代码字体、字号、文章行高、页面宽度、阅读宽度、三档密度、圆角和阴影强度。
- 横幅与图片：默认横幅图片、默认卡片封面、默认头像/角色图、搜索弹窗背景图。
- 博客卡片：摘要字数、分类、标签、评论数、阅读时间。
- 文章页：TOC、更新时间、版权框、过期提示、上一篇/下一篇、代码块折叠高度。
- 恢复默认：按视觉资产、排版与布局密度、预览图片、内容显示分组恢复。点击后先更新预览，只有发布后才保存。
...
贡献代码前请先阅读 [CONTRIBUTING.md](CONTRIBUTING.md)。PR 需要说明是否触碰公开接口、运行了哪些检查，以及 UI、评论、上传、登录、OAuth、设置、PJAX 或发布变更的手动 QA。安全问题请按 [SECURITY.md](SECURITY.md) 私下报告，不要先公开漏洞细节。维护路线图和历史摘要见 [docs/maintenance-notes](docs/maintenance-notes/README.md)；根目录 `task_plan.md`、`findings.md`、`progress.md` 只作为本地工作记忆，不属于公开 PR。
...
```bash
pwsh tools/package-theme.ps1 -Version v0.2.18
```
...
pwsh tools/package-theme.ps1 -OutputName Yneko-Reimu-v0.2.18.zip
...
推荐 tag 命名使用 `vX.Y.Z`，例如 `v0.2.18`。如果手动输入 `0.2.18`，打包脚本会自动补成 `v0.2.18`。
...
`Security settings` enable authentication email protection by default for front-end registration, lost-password, and profile email verification codes, plus native `wp-login.php` registration/lost-password requests. `Security -> Media and privacy` also lets administrators disable comment IP region lookup; when disabled, the theme does not request `ipwho.is` and comment environment badges show only browser and OS information.
...
Enabling TOTP also generates one-time recovery codes. Plain-text codes are shown only once, while the database stores only hashes. Each recovery code can be used once; regenerating recovery codes invalidates the old set, and disabling TOTP clears them.
...
- Visual assets: four core cursor slots, loader image/text/size/rotation, back-to-top icon, and sponsor button icon.
- Typography and layout density: body/heading/code fonts, font sizes, article line height, content width, reading width, three density modes, radius, and shadow strength.
...
- Restore defaults: grouped resets for visual assets, typography/layout density, preview images, and content display. Reset clicks update the preview first and are only saved after publishing.
...
Read [CONTRIBUTING.md](CONTRIBUTING.md) before opening a pull request. PRs should explain public interface impact, checks run, and manual QA for UI, comments, uploads, auth, OAuth, settings, PJAX, or release changes. Report security issues privately through [SECURITY.md](SECURITY.md) instead of opening a public issue first. Public maintenance summaries live in [docs/maintenance-notes](docs/maintenance-notes/README.md); root `task_plan.md`, `findings.md`, and `progress.md` files are local working memory and are not part of public PRs.
...
Current release notes: [v0.2.18](docs/release-notes-v0.2.18.md). Previous release notes: [v0.2.17](docs/release-notes-v0.2.17.md).
