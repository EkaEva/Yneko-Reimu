...
function yneko_reimu_render_settings_auth_security_toggles( $auth_security ) {
	...
	<?php yneko_reimu_settings_group_open( '认证邮件风控', 'Authentication email guard', '默认开启主题级保护，同时覆盖主题前台 AJAX 和 WordPress 原生登录入口。', 'Enabled by default. It covers both theme front-end AJAX and native WordPress login endpoints.' ); ?>
		<div class="yneko-reimu-checkbox-grid">
			<?php foreach ( yneko_reimu_auth_security_toggle_fields() as $field ) : ?>
				<?php yneko_reimu_render_settings_auth_security_toggle( $auth_security, $field ); ?>
			<?php endforeach; ?>
		</div>
...
function yneko_reimu_auth_security_toggle_fields() {
	return array(
		array( 'enabled', '启用认证邮件风控', 'Enable authentication email guard', '1' ),
		array( 'protect_ajax', '保护主题前台 AJAX', 'Protect theme front-end AJAX', '1' ),
		array( 'protect_wp_login', '保护 wp-login.php 注册/忘记密码', 'Protect wp-login.php register/lost password', '1' ),
		array( 'email_alert_enabled', '高危事件邮件报警', 'Email alerts for high-risk events', '0' ),
	);
}

function yneko_reimu_render_settings_auth_security_toggle( $auth_security, $field ) {
	?>
	<label class="yneko-reimu-checkbox-line"><input type="checkbox" name="yneko_reimu_settings[auth_security][<?php echo esc_attr( $field[0] ); ?>]" value="1" <?php checked( '1', $auth_security[ $field[0] ] ?? $field[3] ); ?>> <?php yneko_reimu_admin_bilingual_label( $field[1], $field[2] ); ?></label>
	<?php
}
...
function yneko_reimu_render_settings_security_alerts( $events, $unhandled_count, $mark_url, $clear_url ) {
	...
	<?php yneko_reimu_settings_group_open( '安全报警', 'Security alerts', '最近 100 条认证邮件风控事件会显示在这里。标记已处理只影响角标，不会清除限额计数。', 'The latest 100 authentication email guard events are shown here. Marking handled only affects badges and does not clear rate-limit counters.' ); ?>
		<?php yneko_reimu_render_settings_security_alert_actions( $unhandled_count, $mark_url, $clear_url ); ?>
		<?php yneko_reimu_render_settings_security_alert_list( $events ); ?>
	<?php yneko_reimu_settings_group_close(); ?>
	<?php
}

function yneko_reimu_render_settings_security_alert_actions( $unhandled_count, $mark_url, $clear_url ) {
	?>
...
function yneko_reimu_render_settings_security_alert_list( $events ) {
	if ( ! $events ) {
		?>
...
function yneko_reimu_render_settings_security_alert_card( $event ) {
	...
	<div class="yneko-reimu-security-alert-card<?php echo empty( $event['handled'] ) ? ' is-unhandled' : ''; ?>">
		<?php yneko_reimu_render_settings_security_alert_card_head( $event ); ?>
		<?php yneko_reimu_render_settings_security_alert_card_meta( $event ); ?>
		<?php yneko_reimu_render_settings_security_alert_card_hashes( $event ); ?>
	</div>
	<?php
}

function yneko_reimu_render_settings_security_alert_card_head( $event ) {
	?>
	<div class="yneko-reimu-security-alert-card__head">
		<strong><?php echo esc_html( yneko_reimu_settings_security_event_label( $event ) ); ?></strong>
		<span><?php echo esc_html( date_i18n( 'Y-m-d H:i:s', absint( $event['time'] ?? 0 ) ) ); ?></span>
	</div>
	<?php
}

function yneko_reimu_render_settings_security_alert_card_meta( $event ) {
	?>
	<div class="yneko-reimu-security-alert-card__meta">
		<code><?php echo esc_html( yneko_reimu_settings_security_scope_label( $event ) ); ?></code>
...
function yneko_reimu_render_settings_security_alert_card_hashes( $event ) {
	?>
...
function yneko_reimu_settings_security_event_label( $event ) {
	return function_exists( 'yneko_reimu_auth_security_event_type_label' ) ? yneko_reimu_auth_security_event_type_label( $event['type'] ?? '' ) : ( $event['type'] ?? '' );
}

function yneko_reimu_settings_security_scope_label( $event ) {
	return function_exists( 'yneko_reimu_auth_security_scope_label' ) ? yneko_reimu_auth_security_scope_label( $event['scope'] ?? '' ) : ( $event['scope'] ?? '' );
}
