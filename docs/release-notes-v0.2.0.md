...
- 补齐所有运行时 PHP 模板的 `ABSPATH` 防直访保护，并新增发布就绪检查。
- 新增运行时 `readme.txt`，记录安装、隐私/远程资源、版权来源和许可证口径。
- 将主题兼容声明更新到已验证的 WordPress 7.0 线，并要求发布截图使用 `1200x900` PNG。
...
- Completed `ABSPATH` direct-access guards across runtime PHP templates and added a release-readiness gate.
- Added a runtime `readme.txt` with installation, privacy/remote-resource, credit, and license notes.
- Updated the theme compatibility declaration to the tested WordPress 7.0 line and require a `1200x900` release screenshot.
