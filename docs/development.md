...
The comments/profile JavaScript runtime remains in the main classic `assets/dist/reimu.js` bundle after the v0.2.9 source-only split. The source boundary is `theme/Yneko-Reimu/assets/src/reimu/comments-profile.js`, which owns login, profile, comment, upload, GitHub popup, login-state DOM replacement, reply-form movement, and review-status polling orchestration while still building into the main classic runtime. A future lazy `assets/dist/reimu-comments.js` split is deferred from v0.2.9 and requires the manual checklist in `docs/comments-profile-contract.md`.
