...
The admin settings page also exposes a current-user TOTP management entry under `General -> Account security`. It reuses the profile modal user meta (`_yneko_reimu_totp_secret`, `_yneko_reimu_totp_enabled`, and `_yneko_reimu_totp_pending_secret`) through admin-only AJAX actions, so it must not be treated as a global theme option or a forced site-wide 2FA switch. Once a user enables TOTP, both the front-end comment login AJAX flow and the WordPress `wp-login.php` password-login flow must verify the same current authenticator code. One-time recovery codes are intentionally left for the next round; until then, emergency recovery means removing `_yneko_reimu_totp_enabled` for the locked user outside the theme.
