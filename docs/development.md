...
Admin settings PHP keeps `theme/Yneko-Reimu/inc/settings/page.php` as the registered page callback entrypoint. Focused modules under `theme/Yneko-Reimu/inc/settings/page/` own page context, navigation tabs, the General panel, the floating submit bar, and the hidden admin GIF upload form. Panel bodies are internal renderers in `theme/Yneko-Reimu/inc/settings/panels.php` and focused modules under `theme/Yneko-Reimu/inc/settings/panels/`. The panel entry file keeps shared group/field helpers and smaller panels; high-density panels such as Users, Security, and Music live in their own modules. When changing tabs, panel names, field names, repeatable rows, or review sections, update `tools/check-settings-admin-contract.mjs` in the same change.
