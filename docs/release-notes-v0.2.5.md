...
- 在 `安全设置 -> 媒体与隐私` 新增两个后台开关：允许管理员上传 SVG、显示评论 IP 地区信息。
- SVG 上传默认保持开启且仍只允许管理员；关闭后主题不再为媒体库放行 SVG，`yneko_reimu_allow_svg_uploads` filter 仍保留为开发者最终覆盖点。
- 评论 IP 地区查询默认保持开启；关闭后评论环境标签不再请求 `ipwho.is`，只显示浏览器和系统信息。
- 新增 `npm run check:config-surface`，把配置面按“后台已覆盖 / Customizer 已覆盖 / 开发者扩展 / 内部兼容”分类审计，防止真正适合管理员控制的能力再次遗漏。
...
- 扩展 settings admin 和 comments/profile contract gate，保护新增媒体与隐私设置、SVG 上传 gate 和评论 IP 地区查询 gate。
- 将主题版本、PHP 常量、npm 包版本同步到 `0.2.5`。

### 说明

- 本版本不修改 Rank Math 后台 Analytics、Action Scheduler 或 WP-Cron 行为。
- 本版本不会把所有开发者 filter 都搬进后台；schema、headers、asset strategy、virtual pages 等仍属于开发者扩展面。
...
- Added a `Security -> Media and privacy` group with two backend toggles: administrator SVG uploads and comment IP region info.
- SVG uploads remain enabled by default and administrator-only. Disabling the setting stops the theme from allowing SVG in the Media Library, while the `yneko_reimu_allow_svg_uploads` filter remains the developer final override.
- Comment IP region lookup remains enabled by default. Disabling it prevents `ipwho.is` requests and leaves comment badges limited to browser and OS information.
- Added `npm run check:config-surface` to audit configurable surfaces as covered by admin UI, covered by Customizer, developer extensions, or internal/compatibility values.
...
- Extended the settings admin and comments/profile contract gates to protect the new media/privacy settings, SVG upload gate, and comment IP region lookup gate.
...
- This release does not productize every developer filter into the admin UI; schema, headers, asset strategy, virtual pages, and similar hooks remain developer extension points.
