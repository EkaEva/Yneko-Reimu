...
- 后台账号安全区支持重新生成恢复码，旧恢复码会全部失效；关闭 TOTP 时同步清空恢复码。
...
- The admin account-security panel can regenerate recovery codes, invalidating the old set; disabling TOTP also clears recovery codes.
