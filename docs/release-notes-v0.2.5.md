...
- 本版本暂不包含一次性恢复码；管理员误锁时可临时通过数据库或 WP-CLI 删除该用户的 `_yneko_reimu_totp_enabled` meta，恢复码将在后续版本单独补齐。
...
- One-time recovery codes are not included yet; if an administrator is locked out, remove that user's `_yneko_reimu_totp_enabled` meta through the database or WP-CLI as a temporary recovery path. Recovery codes are planned as a separate follow-up.
